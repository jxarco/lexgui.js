...
export class ColorPicker
{
        ...
        let innerMouseMove = ( e: MouseEvent ) => {
            ...
            if ( dX != 0 && ( mouseX >= 0 || dX < 0 ) && ( mouseX < this.colorPickerBackground.offsetWidth || dX > 0 ) )
            {
                this.intSatMarker.style.left =
                    LX.clamp( parseInt( this.intSatMarker.style.left ) + dX, -this.markerHalfSize,
                        this.colorPickerBackground.offsetWidth - this.markerHalfSize ) + 'px';
            }
                ...
                && ( mouseY < this.colorPickerBackground.offsetHeight || dY > 0 ) )
            {
                this.intSatMarker.style.top =
                    LX.clamp( parseInt( this.intSatMarker.style.top ) + dY, -this.markerHalfSize,
                        this.colorPickerBackground.offsetHeight - this.markerHalfSize ) + 'px';
            }
    ...
    _updateColorValue( newHexValue?: string | null, skipCallback: boolean = false )
    {
        ...
        if ( this.colorModel == 'CSS' )
        {
            ...
            this.labelComponent.set(
                `rgb${this.useAlpha ? 'a' : ''}(${r},${g},${b}${this.useAlpha ? ',' + toFixed( a ) : ''})`
            );
        }
        else if ( this.colorModel == 'Hex' )
        {
            this.labelComponent.set(
                ( this.useAlpha ? this.currentColor.hex : this.currentColor.hex.substr( 0, 7 ) ).toUpperCase()
            );
        }
